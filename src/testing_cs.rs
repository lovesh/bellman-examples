...
#[test]
fn cube_with_testing_cs() {

    // know x such that x^3 + x + 5 == 35
    ...
    let X = cs.alloc(|| "x", || Ok(x)).unwrap();

    let mut x_square = x.clone();
    x_square.square();
    let XSquare = cs.alloc(|| "x^2", || Ok(x_square)).unwrap();
    cs.enforce(
        || "square",
        |lc| lc + X,
        |lc| lc + X,
    ...
    let mut x_cube = x_square.clone();
    x_cube.mul_assign(&x);
    let XCube = cs.alloc(|| "x^3", || Ok(x_cube)).unwrap();
    cs.enforce(
        || "cube",
        |lc| lc + XSquare,
        |lc| lc + X,
    ...
    let five = Fr::from_str("5").unwrap();
    let One = TestConstraintSystem::<Bls12>::one();
    let mut out = x_cube.clone();
    out.add_assign(&x);
    out.add_assign(&five);
    let Out = cs.alloc(|| "x^3 + x + 5", || Ok(out)).unwrap();
    cs.enforce(
        || "out",
        |lc| lc + XCube + X + (five, One),
        |lc| lc + One,
        |lc| lc + Out
    );
